...
##### Идея разработки - самообразование. Проект был сложным и невероятно интригующим. ~~*__на самом деле меня кинула девка я плакал слезами, намертво умирал смертью и устал сидеть в тиндере__*~~
##### Реализовано на: [GoLang](https://github.com/pashanskiy/Tinder-Neural-Network-Like/search?l=go),  [React, TypeScript](https://github.com/search?q=repo%3Apashanskiy%2FTinder-Neural-Network-Like+extension%3Atsx), [Python, Tensorflow](https://github.com/pashanskiy/Tinder-Neural-Network-Like/search?l=python), [gRPC](/grpctransport)
...
### TNN - [GoLang](https://github.com/pashanskiy/Tinder-Neural-Network-Like/search?l=go)
##### Здесь вся логика (Вебсервер, общение с серверами Tinder, защита от "дурака" и тд)
### faceDetect - [Python](https://github.com/pashanskiy/Tinder-Neural-Network-Like/search?l=python)
...
## Автолайк
##### Автолайк делает почти всё тоже-самое, что и загрузка фотографий.
##### За тем лишь отличием, что не сохраняет фотографии, а [предсказывает](/components/autoLikeUsers.go#L124) и отсылает [лайк/дизлайк](/components/autoLikeUsers.go#L186) запросы на сервера тиндера.
...
## И пожалуй последнее
# **АВТОР (Я) НЕ НЕСУ НИКАКОЙ ОТВЕТСТВЕННОСТИ ЗА ИСПОЛЬЗОВАНИЕ ДАННОГО ПРОГРАММНОГО ОБЕСПЕЧЕНИЯ - ИСПОЛЬЗУЙТЕ НА СВОЙ СТРАХ И РИСК!**
